...
from attention.utils.tensor_functions import compute_in_batches

from attention.nets.graph_encoder import GraphAttentionEncoder
from torch.nn import DataParallel
from attention.utils.beam_search import CachedLookup
from attention.utils.functions import sample_many
...
class AttentionModel(nn.Module):
    ...
    def forward(self, input, return_pi=False, nr_routes=0, pi=None):
        """
        ...
        _log_p, pi = self._inner(input, embeddings, nr_routes, pi)
    ...
    def _inner(self, input, embeddings, nr_routes, selectedBefore):
        ...
        i = 0
        while not (self.shrink_size is None and state.all_finished(nr_routes)):
            ...
            if selectedBefore is None:
                # Select the indices of the next nodes in the sequences, result (batch_size) long
                selected = self._select_node(log_p.exp()[:, 0, :], mask[:, 0, :])  # Squeeze out steps dimension
                if i == 0:
                    #enforce anchor to be chosen in first step
